...
class GroupsNotifier extends AsyncNotifier<List<GroupEntity>> {
  ...
  @override
  Future<List<GroupEntity>> build() async => _repo.getAll();
  ...
  Future<void> create({
    ...
    await _repo.create(name: name, icon: icon, color: color);
    refresh();
  }
...
final groupsNotifierProvider =
    AsyncNotifierProvider<GroupsNotifier, List<GroupEntity>>(GroupsNotifier.new);
...
// Check auto-lock (5 min inactivity)
final vaultAutoLockProvider = Provider<bool>((ref) {
  final lockedAt = ref.watch(vaultLockedAtProvider);
  if (lockedAt == null) return false;
  return DateTime.now().difference(lockedAt).inMinutes >= 5;
});
...
final vaultNotifierProvider =
    AsyncNotifierProvider<VaultNotifier, List<VaultEntryEntity>>(VaultNotifier.new);
